...
BEGIN_EVENT_TABLE(PrefsDialog, wxDialog)
	...
	EVT_TEXT    ( XRCID("HelpURL"),          PrefsDialog::OnMiscDirty)
    EVT_SPINCTRL( XRCID("nCalls"),           PrefsDialog::OnMiscDirtySpinEvent)
	EVT_SPINCTRL( XRCID("DebugLevel"),       PrefsDialog::OnMiscDirtySpinEvent)
	...
	EVT_SPINCTRL( XRCID("IaxPttType"),       PrefsDialog::OnIax2DirtySpinEvent)
	EVT_SPINCTRL( XRCID("Iax2Port"),         PrefsDialog::OnIax2DirtySpinEvent)
	EVT_BUTTON  ( XRCID("SaveIax2"),       	 PrefsDialog::OnSaveIax2)
	EVT_BUTTON  ( XRCID("CancelIax2"),     	 PrefsDialog::OnCancel)
	
	EVT_TEXT    ( XRCID("StartupLinks"),     PrefsDialog::OnLinksDirty)
	EVT_TEXT    ( XRCID("PermaLinks"),       PrefsDialog::OnLinksDirty)
	EVT_BUTTON  ( XRCID("SaveLinks"),        PrefsDialog::OnSaveLinks)
	EVT_BUTTON  ( XRCID("CancelLinks"),      PrefsDialog::OnCancel)
...
{    
	...
	HelpURL        = XRCCTRL(*this, "HelpURL",        wxTextCtrl);
    nCalls         = XRCCTRL(*this, "nCalls",         wxSpinCtrl);
	DebugLevel     = XRCCTRL(*this, "DebugLevel",     wxSpinCtrl);
	...
	IaxPttType	   = XRCCTRL(*this, "IaxPttType",     wxSpinCtrl);
	Iax2Port	   = XRCCTRL(*this, "Iax2Port",       wxSpinCtrl);
    SaveIax2       = XRCCTRL(*this, "SaveIax2",       wxButton);
    CancelIax2     = XRCCTRL(*this, "CancelIax2",     wxButton);

	StartupLinks   = XRCCTRL(*this, "StartupLinks",   wxTextCtrl);
	PermaLinks	   = XRCCTRL(*this, "PermaLinks",     wxTextCtrl);
    SaveLinks      = XRCCTRL(*this, "SaveLinks",       wxButton);
    CancelLinks    = XRCCTRL(*this, "CancelLinks",     wxButton);
		
    config->SetPath(_T("/Prefs"));

	StartupLinks->SetValue(config->Read(_T("/Prefs/StartupLinks"), _T("")));
	PermaLinks->SetValue(config->Read(_T("/Prefs/PermaLinks"), _T("")));
	PermaLinks->Disable();
    ...
    config->SetPath(_T("/Prefs"));
	Iax2Port->SetValue(config->Read(_T("Iax2Port"), 4569l));
	IaxPttType->SetValue(config->Read(_T("IaxPttType"), 0l)); 	
	...
	HelpURL->SetValue(config->Read(_T("/Prefs/HelpURL"), _T("")));
    nCalls->SetValue(wxGetApp().nCalls);

	DebugLevel->SetValue(wxGetApp().theFrame->DebugLevel);
...
void PrefsDialog::OnSaveMisc(wxCommandEvent &event)
{
	...
	config->Write(_T("HelpURL"),        HelpURL->GetValue());
    config->Write(_T("nCalls"),         nCalls->GetValue());
	config->Write(_T("DebugLevel"),     DebugLevel->GetValue());
...
void PrefsDialog::OnApplyMisc(wxCommandEvent &event)
{
	...
	wxGetApp().HelpURL      		  = HelpURL->GetValue();
    wxGetApp().nCalls                 = nCalls->GetValue();
	wxGetApp().theFrame->DebugLevel   = DebugLevel->GetValue();
...
void PrefsDialog::OnDoneMisc(wxCommandEvent &event)
{
	Close(TRUE);
}
...
void PrefsDialog::OnIax2DirtySpinEvent(wxSpinEvent &event)
{
    OnIax2Dirty(*((wxCommandEvent *) &event));
}

void PrefsDialog::OnIax2Dirty(wxCommandEvent &event)
{
    SaveIax2->Enable();
    CancelIax2->SetLabel(_T("Cancel"));
}

void PrefsDialog::OnSaveIax2(wxCommandEvent &event)
{
    wxConfig *config = theApp::getConfig();
    config->SetPath(_T("/Prefs"));
	config->Write(_T("Iax2Port"),      Iax2Port->GetValue());
	config->Write(_T("IaxPttType"),    IaxPttType->GetValue());
	SaveIax2->Disable();
	CancelIax2->SetLabel(_T("Done"));
}


void PrefsDialog::OnLinksDirty(wxCommandEvent &event)
{
	SaveLinks->Enable();
}

void PrefsDialog::OnSaveLinks(wxCommandEvent &event)
{
    wxConfig *config = theApp::getConfig();
    config->SetPath(_T("/Prefs"));
	config->Write(_T("StartupLinks"),StartupLinks->GetValue());
	config->Write(_T("PermaLinks"),PermaLinks->GetValue());
	SaveLinks->Disable();
	CancelLinks->SetLabel(_T("Done"));
}
